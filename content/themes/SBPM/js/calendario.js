            ...
            for(i in anio[aniomes])
            {
              ...
              if(aniomesdia)
              {
                ...
                for( j in anio[aniomes][aniomesdia] )
                {
                  ...
                  for ( k in anio[aniomes][aniomesdia][j] ) 
                  {
                    ...
                    else
                    {
                      if(campos.indexOf(anio[aniomes][aniomesdia][j][k]['campo_nombre']) == -1)
                      {
                        ...
                        {
                          otros = otros + "_NEGRILLAINICIO_" + anio[aniomes][aniomesdia][j][k]['campo_nombre'].trim() + ":_NEGRILLAFIN_" + anio[aniomes][aniomesdia][j][k]['contenido'].trim() + '---';
                        }
                  ...
                  if (fecha != "" && fechaAux.isValid())
                  {
					  
                    
                    //diames   = fechaAux.getDay();
                    
                    //console.log(fechaAux.isValid()); 
...
//                    alert(minutos);
                  ...
                  else
                  {
                    ...
                    anooficial = ano;
                    mesoficial = meshoy;
                    mesanio = mesoficial + "/" + fecha.substring(0,4);
                    mesanio = mesanio.toUpperCase();
                    diaevento  = diahoy;
                  }
...
//            jQuery("#nombre_dia").html('<h3>'+meses[parseInt(h)]+'</h3>');
      ...
      else
      {
       // html = "<h1 style='color: #4a5eaf; font-family: roboto; font-weight: 300; font-style: Italic; font-size:20px'>No hay eventos programados</h1>";
        html = "<h1 style='color: #4a5eaf; font-family: roboto; font-weight: 300; font-style: Italic; font-size:20px'>La información aún no ha sido ingresada. Regresa más tarde.</h1>";
        jQuery(".calendar-months").html(html);
      }
}
...
{
    window.scrollTo(0, 0);
    jQuery("#calendario-principal").show();
    jQuery("#detalle-calendario").empty();  
...
//    facebook   = '';
    ...
    for (index = 0; index < otros.length; index++) {
        ...
        otros_negrilla = otros[index].trim().replace(/_NEGRILLAINICIO_/g,"<b>").replace(/_NEGRILLAFIN_/g,"</b>");
    ...
    jQuery("#calendario-principal").hide();
    jQuery("#detalle-calendario").empty();
    ...
    jQuery("#detalle-calendario").show();
    if(latitud!="nothing")
    {
